angular
	.module('app')
	.controller('ModalPagarCtrl', ModalPagarCtrl);

function ModalPagarCtrl($uibModalInstance, $uibModal, items, cajasFactory, $state, $rootScope, ngNotify) {
	var vm = this;
	vm.cancel = cancel;
	...
	function ok() {
		if (vm.casePago == "" || vm.casePago == undefined) {
			ngNotify.set('Por favor llena un metodo de pago.', 'error');
		} else if (vm.monto == undefined || vm.monto == "") {
			ngNotify.set('Saldo erroneo.', 'error');
		} else {
			switch (vm.casePago) {
				case 1:
					...
					if (vm.selectedBancoCheque.Clave == 0) {
						...
						if (vm.dineroCheque == vm.monto) {
							...
							cajasFactory.insertSeguridadToken(items.IdSession).then(function(data) {
								cajasFactory.grabaPago(objPagar).then(function(data) {
									$uibModalInstance.dismiss('cancel');
									$rootScope.$emit("ocultarPagar", {});
									$rootScope.$emit("getVendedores", {});
									ngNotify.set('Pago Exitoso', 'success');
